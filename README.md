...
The script runs locally and authenticates to Azure via your active `az login` session. It supports two backends: **Azure OpenAI direct** (API key, any Entra ID tenant) and **Microsoft Foundry** (`az login` credentials, home tenant only). Both connect to GPT-4.1 hosted in Microsoft Foundry, Microsoft's AI platform on Azure. Azure OpenAI is the default for cross-tenant troubleshooting; use the Foundry backend during development for its built-in OpenTelemetry tracing to Application Insights.
